...
function Dashboard() {
  ...
  return (
    <div style={{ position: 'relative', background: '#0d0c0d', minHeight: '100vh', display: 'flex' }}>
      ...
      {/* Sidebar — fixed so it never scrolls (hidden on mobile, MobileSidebar renders separately) */}
      <div style={{ position: 'fixed', top: 0, left: 0, height: '100vh', zIndex: 10, display: isMobile ? 'none' : 'block' }}>
      ...
      <div style={{ flex: 1, position: 'relative', zIndex: 1, overflowY: 'auto', padding: isMobile ? '16px' : '24px', marginLeft: isMobile ? '0' : (sidebarOpen ? '300px' : '80px'), transition: 'margin-left 0.3s cubic-bezier(0.4,0,0.2,1)' }}>
          ...
          {activePanel === "profile" && (
            <div
              ...
              style={{
                ...
                padding: isMobile ? '16px' : '40px', cursor: 'pointer',
                marginLeft: isMobile ? '0' : (sidebarOpen ? '300px' : '80px'),
      ...
      {activePanel === "contact" && (
        <div
          ...
          style={{
            ...
            padding: isMobile ? '16px' : '40px', cursor: 'pointer', gap: isMobile ? '16px' : '24px',
            marginLeft: isMobile ? '0' : (sidebarOpen ? '300px' : '80px'),
            transition: 'margin-left 0.3s cubic-bezier(0.4,0,0.2,1)'
          }}
        >
          <div onClick={e => e.stopPropagation()} style={{ textAlign: 'center', cursor: 'default' }}>
            <BlurText text="neelanshdosi@gmail.com" delay={100} animateBy="words" direction="top" stepDuration={0.4} className={`${isMobile ? 'text-xl' : 'text-4xl'} font-semibold text-white`} />
          </div>
          <div onClick={e => e.stopPropagation()} style={{ textAlign: 'center', cursor: 'default' }}>
            <BlurText text="8319122246" delay={80} animateBy="characters" direction="top" stepDuration={0.35} className={`${isMobile ? 'text-xl' : 'text-4xl'} font-semibold text-white tracking-widest`} />
          </div>
          <div onClick={e => e.stopPropagation()} style={{ display: 'flex', gap: isMobile ? '12px' : '20px', flexWrap: 'wrap', justifyContent: 'center' }}>
            <a href="https://www.linkedin.com/in/neelansh-dosi-866777282" target="_blank" rel="noopener noreferrer"
              style={{ display: 'flex', alignItems: 'center', gap: '10px', color: '#fff', textDecoration: 'none', background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '999px', padding: isMobile ? '8px 16px' : '10px 24px', fontSize: isMobile ? '0.875rem' : '1rem', fontWeight: 600, transition: 'all 0.2s', backdropFilter: 'blur(8px)' }}
        ...
        >
          <div onClick={e => e.stopPropagation()} style={{ maxWidth: '680px', textAlign: 'center', cursor: 'default' }}>
            <BlurText
              ...
              className={`${isMobile ? 'text-xl' : 'text-4xl'} font-semibold leading-relaxed text-white`}
            />
